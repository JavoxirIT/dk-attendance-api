<?php
	header("Access-Control-Allow-Origin: *");
	header("Content-Type: application/json; charset=UTF-8");
	header("Access-Control-Allow-Headers: Content-Type, Access-Control-Allow-Headers, Authorization, X-Requested-With");
	header("Access-Control-Allow-Methods: POST, GET, DELETE, UPDATE, OPTIONS");
	
	include_once 'config.php';
	$data = file_get_contents("php://input");
	$ret = [];
	$arr = json_decode($data);
	if($arr){
		$fio = $arr->fio;
	    $tel = $arr->tel;
	    $arrival_date = $arr->arrival_date; 
		$party_id = $arr->party_id;
	    $class_time_id = $arr->class_time_id;
	    $training_days_id = $arr->training_days_id;
			$sql = mysqli_query($link,"INSERT INTO student (fio,tel,arrival_date,party_id,class_time_id,training_days_id) VALUES ('$fio','$tel','$arrival_date','$party_id','$class_time_id','$training_days_id')");
			if($sql){
				$result = mysqli_query($link,"SELECT distinct stud.*, 
					par.party_name, 
					les.start_time, les.end_time, 
					tra.days as training_days FROM student stud 
					inner join party par on stud.party_id = par.id 
					inner join lesson_time les on les.id = stud.class_time_id 
					inner join training_days tra on tra.id = stud.training_days_id where stud.id > 0 order by stud.id desc;" 
				);
				$data = $result->fetch_all(MYSQLI_ASSOC);
				
				$ret += ["statusCode" => 201, "message" => "Новый учиник успешно добавлен", "student" => $data];
			}
			else{
				$ret += ["statusCode" => 405, "message" => "Ошибка при сохранении", "arr"=>$arr];
			}
	}
	else{
		$ret += ["statusCode" => 400, "message" => "Нет данных"];
	}
	echo json_encode($ret);
?>